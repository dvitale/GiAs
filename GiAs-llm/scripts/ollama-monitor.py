...
class HTUI:
    ...
    def draw_bar(self, y, x, width, pct, label):
        ...
        self.stdscr.addstr(y, x + len(label) + 1, "[")
        if filled > 0:
            self.stdscr.addstr(y, x + len(label) + 2, "|" * filled, color)
        if filled < bar_w:
            self.stdscr.addstr(y, x + len(label) + 2 + filled, " " * (bar_w - filled))
        self.stdscr.addstr(y, x + len(label) + 2 + bar_w, "]")
        if pct is not None:
            self.stdscr.addstr(y, x + len(label) + 4 + bar_w, f" {pct:5.1f}%", color)
    ...
    def draw(self):
        ...
        line = 0
        self.stdscr.addstr(line, 0, f"Ollama Monitor  |  {now}  |  Load {load1} {load5} {load15}")
        line += 1
        if up is not None:
            up_h = int(up // 3600)
            up_m = int((up % 3600) // 60)
            up_s = int(up % 60)
            self.stdscr.addstr(line, 0, f"Uptime: {up_h}h {up_m}m {up_s}s  |  Refresh: {self.interval:.1f}s")
        line += 2
        ...
        if mem:
            ...
            if used is not None:
                self.stdscr.addstr(line, maxx - 26, f"{fmt_bytes(used)}/{fmt_bytes(total)}")
            line += 1
            ...
            if swt and swf is not None:
                ...
                self.stdscr.addstr(line, maxx - 26, f"{fmt_bytes(su)}/{fmt_bytes(swt)}")
                line += 2

        rx_tx = self._net_rate()
        if rx_tx:
            rx, tx = rx_tx
            self.stdscr.addstr(line, 0, f"NET: RX {fmt_rate(rx)}  TX {fmt_rate(tx)}")
            line += 1

        dr = self._disk_rate()
        if dr:
            rr, wr = dr
            self.stdscr.addstr(line, 0, f"DISK IO: Read {fmt_rate(rr)}  Write {fmt_rate(wr)}")
            line += 2

        # Ollama process
        pid = get_ollama_pid()
        if not pid:
            self.stdscr.addstr(line, 0, "Ollama: NOT RUNNING", self._color("R"))
            line += 2
        else:
            psf = get_ps_fields(pid)
            self.stdscr.addstr(line, 0, f"Ollama PID {pid}", self._color("G"))
            line += 1
            if psf:
                cpu_p = float(psf["pcpu"])
                mem_p = float(psf["pmem"])
                self.stdscr.addstr(line, 0, f"CPU {cpu_p:.1f}%  MEM {mem_p:.1f}%  RSS {fmt_bytes(psf['rss'])}  VSZ {fmt_bytes(psf['vsz'])}")
                line += 1
                self.stdscr.addstr(line, 0, f"Threads {psf['nlwp']}  Uptime {psf['etime']}")
                line += 1
                cmd = psf["cmd"][: maxx - 1]
                self.stdscr.addstr(line, 0, f"Cmd: {cmd}")
                line += 1
            io_rate = self._proc_io_rate(pid)
            if io_rate:
                rr, wr = io_rate
                self.stdscr.addstr(line, 0, f"Proc IO: Read {fmt_rate(rr)}  Write {fmt_rate(wr)}")
                line += 1
            total_conn, estab, clients = count_active_connections()
            self.stdscr.addstr(line, 0, f"Connessioni 11434: {estab} attive, {total_conn} totali")
            line += 2

        # GPU
        gpus = get_nvidia_stats()
        if gpus:
            self.stdscr.addstr(line, 0, "GPU NVIDIA:", self._color("B"))
            line += 1
            for idx, g in enumerate(gpus):
                used = float(g["mem_used"]) if g["mem_used"].isdigit() else None
                total = float(g["mem_total"]) if g["mem_total"].isdigit() else None
                pct = (used / total * 100.0) if used and total else None
                self.stdscr.addstr(line, 0, f"GPU{idx} {g['name']} | Util {g['util']}% | Mem {g['mem_util']}% | Temp {g['temp']}C | Power {g['power']}W")
                line += 1
                if pct is not None:
            ...
            rocm = get_rocm_stats()
            if rocm:
                self.stdscr.addstr(line, 0, "GPU AMD (rocm-smi):", self._color("B"))
                line += 1
                for ln in rocm.splitlines()[:4]:
                    self.stdscr.addstr(line, 0, ln[: maxx - 1])
                    line += 1
                line += 1
        ...
        self.stdscr.addstr(line, 0, "Modelli:", self._color("B"))
        line += 1
        if installed:
            self.stdscr.addstr(line, 0, "Installati: " + ", ".join([m[0] for m in installed[:5]]) + (" ..." if len(installed) > 5 else ""))
        else:
            self.stdscr.addstr(line, 0, "Installati: nessuno o comando non disponibile")
        line += 1

        if ps_json and ps_json.get("models"):
            models = ps_json["models"]
            names = [m.get("name", "?") for m in models]
            self.stdscr.addstr(line, 0, "Caricati: " + ", ".join(names[:4]) + (" ..." if len(names) > 4 else ""))
            line += 1
        else:
            self.stdscr.addstr(line, 0, "Caricati: nessuno" if ps_json else "Caricati: API non raggiungibile")
            line += 1
        ...
        if estab > 0:
            if ps_json and ps_json.get("models") and len(ps_json["models"]) == 1:
                ...
                self.stdscr.addstr(line, 0, f"Attivo: {active}", self._color("G"))
            else:
                self.stdscr.addstr(line, 0, "Ollama sta servendo richieste", self._color("Y"))
        else:
            self.stdscr.addstr(line, 0, "Nessuna richiesta attiva", self._color("B"))
        line += 1

        if ps_ms is not None or tags_ms is not None:
            self.stdscr.addstr(line, 0, f"API latency: /ps {ps_ms:.0f}ms  /tags {tags_ms:.0f}ms" if ps_ms is not None and tags_ms is not None else "API latency: n/a")
            line += 2
        ...
        if clients:
            ...
            self.stdscr.addstr(line, 0, f"Top client: {top_str}")
            line += 2
        ...
        # Logs
        if self.log_path:
            self.stdscr.addstr(line, 0, f"Log tail ({self.log_path}):", self._color("B"))
            line += 1
            for ln in tail_file(self.log_path, 4):
                self.stdscr.addstr(line, 0, ln[: maxx - 1])
                line += 1
        else:
            self.stdscr.addstr(line, 0, "Log: nessun file trovato", self._color("Y"))
            line += 1

        self.stdscr.addstr(maxy - 1, 0, "q per uscire | Ctrl+C per uscire")
        self.stdscr.refresh()
